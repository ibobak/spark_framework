...
C_FORMAT_MAP = {C_CSV_GZ: "csv", C_JSON_GZ: "json", C_AVRO: "com.databricks.spark.avro"}
...
C_PG_URL_PARSER = re.compile(
    r"jdbc:postgresql://([a-zA-Z0-9.-]+)(:(\d+))(/([a-zA-Z0-9.-]+)).*"
)
...
def parse_conn_info(a_conn_info: dict):
    ...
    if C_TYPE not in a_conn_info:
        raise ValueError(
            f"You need to specity 'type' with one of the following values: {C_SUPPORTED_CONN_TYPES}"
        )
    ...
    if result_type not in C_SUPPORTED_CONN_TYPES:
        raise ValueError(
            f"The following connection types are supported: {C_SUPPORTED_CONN_TYPES}"
        )

    # check if necessary options were passed
    if not check_keys(C_MUST_HAVE_OPTIONS[result_type], result_options):
        raise ValueError(
            f"You need to specify all these options for {result_type}: {C_MUST_HAVE_OPTIONS[result_type]}"
        )
...
def get_fs_reader(a_format, a_header, a_schema, a_verbose_level=3, **a_options):
    ...
    if a_format not in C_FS_SUPPORTED_FORMATS:
        raise ValueError(
            f"a_format must be one of the following: {C_FS_SUPPORTED_FORMATS}"
        )
    if a_format in [C_CSV_GZ, C_CSV]:
        if not isinstance(a_header, bool):
            raise ValueError(
                f"For formats {C_CSV_GZ} or {C_CSV} you need to set a_header=True/False explicitly"
            )
    else:
    ...
    if a_schema:
        ...
        print_verbose(4, a_verbose_level, "no schema specified, inferring")

    # other options
    if a_options:
        print_verbose(
            4, a_verbose_level, f"other options: {hide_sensitive_str(a_options)}"
        )
...
def get_jdbc_reader(
    a_table, a_query, a_table_options, a_url, a_verbose_level=3, **a_options
):
    ...
    if a_table_options:
        print_verbose(
            4, a_verbose_level, f"other options: {hide_sensitive_str(a_table_options)}"
        )
...
def load(
    ...
    if isinstance(a_table, dict):
        if a_format is not None or a_header is not None or a_schema is not None:
            raise ValueError(
                "If you specify the a_table to be a dictionary of values, then you should NOT specify "
        ...
        raise ValueError(
            "a_table must be either dict(name, format, header, schema) or string or list"
        )
    ...
    if source_type == C_DFS or source_type == C_LOCAL:
        ...
        path = (
            join_path_generic(source_url, a_table)
            if source_type == C_DFS
            else join_path_os(source_url, a_table)
        )
        print_verbose(
            1, a_verbose_level, f"loading dataframe from {source_type} path {path}"
        )
        stat_path.append((datetime.now(), path))
        result = get_fs_reader(
            a_format,
            a_header,
            a_schema,
            a_verbose_level=a_verbose_level,
            **a_table_options,
        ).load(path)
        ...
        print_verbose(
            1,
            a_verbose_level,
            f"loading dataframe from table {a_table} at {source_url}",
        )
        result = get_jdbc_reader(
            a_table,
            a_query,
            a_table_options,
            source_url,
    ...
    if a_show:
        ...
        if a_show_order_by:
            result_to_show = change(
                result_to_show, a_order_by=a_show_order_by, a_verbose_level=0
            )
...
def pg_make_copy_to_func(a_columns, a_string_fields, a_dest_table, **a_pg_conn_params):
    ...
    def result_func(a_rows):
        ...
        if len(a_string_fields) > 0:
            mgr.copy(
                tuple(
                    r[c].encode("utf-8") if c in a_string_fields and r[c] else r[c]
                    for c in a_columns
                )
                for r in a_rows
            )
...
def pg_copy(
    ...
    **a_postgres_conn_params2,
    ...
    if C_URL in conn_params:
        if (
            C_HOST in conn_params
            or C_PORT in conn_params
            or C_DATABASE in conn_params
            or C_DBNAME in conn_params
        ):
            raise ValueError(
                f"If you set the url, then there should be no {C_HOST}, {C_PORT}, {C_DATABASE} or {C_DBNAME} in connection parameters"
            )
        conn_params[C_HOST], conn_params[C_PORT], conn_params[C_DATABASE] = (
            parse_pg_url(conn_params[C_URL])
        )
    ...
    a_df.foreachPartition(
        pg_make_copy_to_func(a_df.columns, string_fields, a_dest_table, **conn_params)
    )
...
def get_fs_writer(
    ...
    **a_options,
    ...
    if a_format in [C_CSV_GZ, C_CSV]:
        if not isinstance(a_header, bool):
            raise ValueError(
                f"For formats {C_CSV_GZ} and {C_CSV} you need to set a_header=True/False explicitly"
            )
    else:
        # for non-csv format there should be no header or schema
        if isinstance(a_header, bool):
    ...
    if a_format in [C_CSV_GZ, C_CSV]:
        ...
        print_verbose(
            4,
            a_verbose_level,
            "options ignoreTrailingWhiteSpace=False, ignoreTrailingWhiteSpace=False",
        )
        res = res.option("ignoreLeadingWhiteSpace", False).option(
...
def save(
    ...
    if a_table is None:
        raise ValueError(
            "The a_table must be set (or a_table.name must be set, if a_table is a dictionary)"
        )

    # parameters compatibility checks
    if a_columns_lowercase and a_columns_rename_map:
        raise ValueError(
            "You cannot set a_columns_lowercase=True and a_columns_rename simultaneously"
        )
    ...
    if dest_type == C_DFS or dest_type == C_LOCAL:
        path = (
            join_path_generic(dest_url, a_table)
            if dest_type == C_DFS
            else join_path_os(dest_url, a_table)
        )
        print_verbose(
            1,
            a_verbose_level,
            f"saving to {dest_type} path {path}, started at: {date_to_ymd_hms(dtn)}",
        )
        get_fs_writer(
            df,
            a_format,
        ...
        if a_fast_write and dest_url.lower().startswith("jdbc:postgresql"):
            print_verbose(
                1,
                a_verbose_level,
                f"saving dataframe to table {a_table} of {dest_url} (using PostgreSQL copy routine), started at: {date_to_ymd_hms(dtn)}",
            )
            ...
            print_verbose(
                1,
                a_verbose_level,
                f"saving to table {a_table} of {dest_url} (using regular JDBC), started at: {date_to_ymd_hms(dtn)}",
            )
            get_jdbc_writer(
                df,
                dest_url,
        ...
        raise NotImplementedError(f"Type {dest_type} is not implemented yet")

    sp, dtn = seconds_passed(dtn, True)
    print_verbose(
        1,
        a_verbose_level,
        f"dataframe saved, seconds passed: {sp}, finished at: {date_to_ymd_hms(dtn)}",
    )
...
def cache(
    a_df: DataFrame,
    a_row_count=False,
    a_storage_level=StorageLevel.MEMORY_ONLY,
    a_temp_table=None,
    a_verbose_level=3,
):
    ...
    cnt = -1
    if a_row_count:
        print_verbose(1, a_verbose_level, "caching dataframe and counting rows")
        cnt = a_df.count()
        print_verbose(1, a_verbose_level, f"done caching, row count: {cnt:,}.")
    else:
        print_verbose(1, a_verbose_level, "done caching.")
    if a_temp_table:
...
def unpersist(a_df: DataFrame, a_force=False, a_verbose_level=3):
    ...
    if a_force:
        print_verbose(1, a_verbose_level, "dataframe is unpersisted forcibly")
    elif version_greater_240:
        print_verbose(1, a_verbose_level, "dataframe is unpersisted")
    else:
        print_verbose(
            1,
            a_verbose_level,
            "dataframe is NOT unpersisted because Spark version is less than 2.4.0, and it will lead to cascaded cache invalidation",
        )
...
def distinct(
    ...
    a_return_count=False,
    a_verbose_level=3,
) -> Union[DataFrame, Tuple[DataFrame, int]]:
    ...
    if a_cache:
        cnt = cache(
            df_result,
            a_row_count=a_row_count or a_return_count,
            a_verbose_level=a_verbose_level,
        )
    ...
    if a_row_count or a_return_count:
        if cnt < 0:
            cnt = df_result.count()
            print_verbose(
                1, a_verbose_level, f"distinct row count (no caching): {cnt:,}"
            )
...
def min_value(a_df: DataFrame, a_column: str):
    pdf1 = sql(
        f"select min({a_column}) as {a_column} from [0]", a_df, a_verbose_level=5
    ).toPandas()
...
def max_value(a_df: DataFrame, a_column: str):
    pdf1 = sql(
        f"select max({a_column}) as {a_column} from [0]", a_df, a_verbose_level=5
    ).toPandas()
    return pdf1[a_column].iloc[0]
...
def collect_values(a_df: DataFrame, a_column, a_order=False, a_verbose_level=3):
    rows = change(
        a_df,
        a_select=[a_column],
        a_order_by=a_column if a_order else None,
        a_verbose_level=a_verbose_level,
    ).collect()
    return [r[a_column] for r in rows]
...
def groupby_count(
    ...
    cnt_expression = (
        "count(*)"
        if a_distinct_column is None
        else f"count(distinct {a_distinct_column})"
    )
    ...
    if a_rc_desc:
        if order_by is None:
            ...
            order_by[rc_col_name] = "desc"
    order_query = (
        " order by " + ", ".join(f"{c} {v}" for c, v in order_by.items())
        if order_by
        else ""
    )

    query = (
        f"select {cols}, {cnt_expression} as {rc_col_name} from {table_name} group by {cols}"
        + order_query
    )
...
def double_groupby_count(a_df, a_group_cols, a_count_col_name=None):
    ...
    count_col_name = (
        a_count_col_name if a_count_col_name else f"rows_for_{g_str.replace(', ', '_')}"
    )

    return sql(
        f"""with x as (select {g_str}, count(*) as {count_col_name} from [0] group by {g_str})
select {count_col_name}, count(*) as RC from x group by {count_col_name} order by {count_col_name}""",
        a_df,
    )
...
def groupby_sum(
    a_df: DataFrame,
    a_group_columns: ListOrStr,
    a_sum_columns: ListOrStr,
    a_order=False,
    a_verbose_level=3,
) -> DataFrame:
...
def groupby(
    ...
    if not isinstance(a_columns, dict):
        raise ValueError(
            "a_coumns must be a dict of 'column': 'sum', 'column2': 'max', etc."
        )
    if (
        a_order_by is not None
        and (not isinstance(a_order_by, dict) and not isinstance(a_order_by, list))
        and not isinstance(a_order_by, str)
    ):
    ...
    if a_order_by_group:
        ...
        if isinstance(a_order_by, dict):
            order_query = " order by " + ", ".join(
                f"{c} {o}" for c, o in a_order_by.items()
            )
        else:
    ...
    query = (
        f"select {group_cols}, {agg_cols} from {table_name} group by {group_cols}"
        + order_query
    )
    return sql(query, a_verbose_level=a_verbose_level)
...
def get_change_query(
    ...
    a_distinct=False,
    a_filter_table=None,
    a_filter_columns=None,
    a_filter_not_table=None,
    a_filter_not_columns=None,
    a_where=None,
    a_select_end=None,
    a_drop_end=None,
    a_order_by=None,
    ...
    if a_filter_table and (
        not isinstance(a_filter_columns, list) or len(a_filter_columns) == 0
    ):
        raise ValueError(
            "when a_filter_table is not None, a_filter_columns should be a list"
        )
    if a_filter_not_table and (
        not isinstance(a_filter_not_columns, list) or len(a_filter_not_columns) == 0
    ):
        raise ValueError(
            "when a_filter_not_table is not None, a_filter_not_columns should be some list"
        )
    ...
    query = "select {}{} from {}".format(
        ...
        ", ".join(
            safe_col_name(c) if c == v else "{} as {}".format(v, safe_col_name(c))
            for (c, v) in cols
        ),
        a_input_table,
    )
    ...
    if a_select_end and len(a_select_end) > 0:
        query = (
            f"select {comma_columns(a_select_end)} from (\n\t" + query + "\n) as zs "
        )
    ...
    if a_order_by and len(a_order_by) > 0:
        query = query + "\norder by {}".format(
            ",".join("{} {}".format(safe_col_name(c), v) for c, v in a_order_by.items())
        )
...
def change(
    ...
    if a_replace is None:
        ...
        if bad_column_names:
            raise ValueError(
                f"a_replace parameter contains columns which are missing in the dataframe: {bad_column_names}"
            )
    if a_rename is None:
        a_rename = {}
    else:
        ...
        if bad_column_names:
            raise ValueError(
                f"a_rename parameter contains columns which are missing in the dataframe: {bad_column_names}"
            )
    if a_add is None:
        a_add = {}
    if a_drop_end is None:
    ...
    print_verbose(
        1, a_verbose_level, "changing dataframe, operations: " + ", ".join(operations)
    )
    ...
    filter_table = (
        temp_table(a_filter_df, a_prefix="df_filter") if a_filter_df else None
    )
    filter_not_table = (
        temp_table(a_filter_not_df, a_prefix="df_not_filter")
        if a_filter_not_df
        else None
    )
    ...
    query = get_change_query(
        ...
        a_filter_table=filter_table,
        a_filter_columns=a_filter_columns,
        a_filter_not_table=filter_not_table,
        a_filter_not_columns=a_filter_not_columns,
        a_where=a_where,
        a_select_end=a_select_end,
        a_drop_end=a_drop_end,
        a_order_by=a_order_by,
    )
    ...
    if a_print_limit is not None and a_print_limit > 0:
        print_verbose(
            4,
            a_verbose_level,
            f"final query (firtst {a_print_limit} characters):\n"
            + query[:a_print_limit]
            + "... [cut off]",
        )
    else:
        print_verbose(4, a_verbose_level, "final query:\n" + query)
    result = sql(
        query,
        a_cache=a_cache,
        a_row_count=a_row_count,
        a_temp_table=a_temp_table,
        a_verbose_level=0,
    )
    ...
    if a_select_end:
        ...
        if missing:
            raise ValueError(
                f"Bad a_select_end value: columns {missing} are missing in the resulting dataframe. Full list of columns: {result.columns}"
            )
...
def join_clause(
    ...
    if a_handle_null_columns is None:
        return " and ".join(
            f"{a_left_prefix}.{safe_col_name(c)}={a_right_prefix}.{safe_col_name(c)}"
            for c in a_fields
        )
    ...
    for c, dt in zip(a_fields, a_join_col_types):
        if c not in a_handle_null_columns:
            ...
            join_list.append(
                f"coalesce({a_left_prefix}.{safe_col_name(c)}, {null_clause})=coalesce({a_right_prefix}.{safe_col_name(c)}, {null_clause})"
            )
...
def join(
    ...
    if a_how not in alllowed_joins:
        raise ValueError(
            f"The join type {a_how} is not in allowed joins: {alllowed_joins}"
        )
    ...
    cols1 = (
        a_df1.columns
        if not a_drop_cols1
        else [c for c in a_df1.columns if c not in a_drop_cols1]
    )
    cols2 = (
        a_df2.columns
        if not a_drop_cols2
        else [c for c in a_df2.columns if c not in a_drop_cols2]
    )
    ...
    first_cols = [
        "df1.{}".format(safe_col_name(c)) for c in cols1 if c not in join_cols
    ]
    second_cols = [
        "df2.{}".format(safe_col_name(c))
        for c in cols2_minus_cols1
    ...
    """.format(
        ...
        join_clause(
            ...
            "df1",
            "df2",
            a_join_col_types=join_col_types,
            a_handle_null_columns=a_handle_null_columns,
        ),
    )
    if a_where:
    ...
    if a_row_count or a_return_count:
        ...
        if a_old_count:
            ...
            print_verbose(
                1,
                a_verbose_level,
                f"old record count: {a_old_count:,}; new record count: {new_cnt:,}; dropped {dropped:,} {dropped_percent} records.",
            )
        else:
    ...
    for i, df in enumerate(a_df_list):
        if schema_types is None:
            schema_types = [
                ...
                if a_columns is None or f.name in a_columns
            ]
        else:
            schema_types2 = [
                str(f.dataType)
            ...
            if schema_types2 != schema_types:
                raise ValueError(
                    f"Schema of the {i + 1}th dataframe is different. Expected: {schema_types}, actual: {schema_types2}"
                )
...
def moving_average(
    ...
    a_window,
    a_min_periods=1,
    a_return_all_columns=True,
):
    # DO NOT REMOVE select(*a_df.columns):  this is related to inability to add column dynamically to it
    schema = (
        a_df.select(*a_df.columns).schema
        if a_return_all_columns
        else a_df.select(a_group_col, a_sort_col, a_value_col).schema
    )
    schema = schema.add(StructField(a_moving_avg_col, DoubleType()))

    @pandas_udf(schema, PandasUDFType.GROUPED_MAP)
    def ma(a_pdf):
        a_pdf[a_moving_avg_col] = (
            a_pdf.sort_values(a_sort_col)[a_value_col]
            .rolling(window=a_window, min_periods=a_min_periods)
            .mean()
        )
        return a_pdf
...
def exp_moving_average(
    ...
    def ema(a_pdf):
        a_pdf[a_moving_avg_col] = (
            a_pdf.sort_values(a_sort_col)[a_value_col]
            .ewm(span=a_span, min_periods=a_min_periods)
            .mean()
        )
        return a_pdf
...
def median(
    a_df, a_part_columns, a_column, a_new_column_name, a_row_count=False, a_cache=False
):
    if isinstance(a_part_columns, str):
        a_part_columns = [a_part_columns]
    w = Window.partitionBy(*a_part_columns).orderBy(a_column)
    df_result = (
        a_df.withColumn("rank", sf.row_number().over(w))
        .withColumn(
            "count_row_part",
        ...
        .withColumn(
            "even_flag",
            sf.when(sf.col("count_row_part") % 2 == 0, sf.lit(1)).otherwise(sf.lit(0)),
        )
        .withColumn("mid_value", sf.floor(sf.col("count_row_part") / 2))
        .withColumn(
            "avg_flag",
            sf.when(
                ...
                sf.lit(1),
            ).otherwise(sf.when(sf.col("rank") == sf.col("mid_value") + 1, sf.lit(1))),
        )
        .filter(sf.col("avg_flag") == 1)
        .drop("avg_flag")
        .groupby(a_part_columns)
        .agg(sf.avg(sf.col(a_column)).alias(a_new_column_name))
    )
    if a_cache and a_row_count:
...
def calc_stat_local(
    ...
    if a_round_to_decimal_places != -1 and a_trunc_to_decimal_places != -1:
        raise ValueError(
            "yout should use either a_round_to_decimal_places or a_trunc_to_decimal_places, but not both"
        )
    ...
    calc_ms = (
        "mean_minus_1std" in a_stats
        or "mean_minus_2std" in a_stats
        or "mean_minus_3std" in a_stats
        or "mean_plus_1std" in a_stats
        or "mean_plus_2std" in a_stats
        or "mean_plus_3std" in a_stats
    )
    ...
    calc_quantiles = (
        a_quantiles
        if not calc_iqr
        else sorted(list(set(a_quantiles).union({0.25, 0.75})))
    )
    ...
    for c, p in a_column_prefix_map.items():
        ...
        if calc_iqr:
            pdf_res["q25_minus_15iqr"] = pdf_res["q25"] - 1.5 * (
                pdf_res["q75"] - pdf_res["q25"]
            )
        ...
        pdf_res_all = (
            pdf_res
            if pdf_res_all is None
            else pd.merge(pdf_res_all, pdf_res, left_index=True, right_index=True)
        )
...
def statistics(
    ...
    for _, p in a_column_prefix_map.items():
        field_names.extend(
            [p + f for f in stats] + [p + "q" + f"{q:.2f}"[2:] for q in quantiles]
        )
    ...
    def stat_func2(a_pdf):
        return calc_stat_local(
            a_pdf,
            stats,
            quantiles,
            a_column_prefix_map,
            a_group_columns,
            a_return_uppercase=a_return_uppercase,
            a_reset_index=True,
    ...
    df_result = a_df.groupBy(
        a_split_group_columns if a_split_group_columns else a_group_columns
    ).applyInPandas(stat_func2, schema)
...
def get_schema_columns_string(a_df, a_columns=None):
    ...
    for gc in a_columns:
        if gc not in schema_dict:
            raise ValueError(f"Column {gc} is not found in the schema")
        result.append(
            schema_dict[gc].name + " " + str(schema_dict[gc].dataType).lower()[:-4]
        )
    return ", ".join(result)
...
def correlation(
    ...
    if a_type.lower() not in known_corr_types:
        raise ValueError(
            f"Unknown correlation type {a_type}, possible types: {known_corr_types}"
        )
...
def show_values(a_df, a_pdf_dc=None, a_limit=10):
    ...
    for f, dc in zip(pdf["COL_NAME"].values, pdf["DC"].values):
        if dc > 0 and dc < 100000:
            pdf_g = sql(
                f"select {f}, count(*) from {t} group by {f} order by count(*) desc"
            ).toPandas()
            if dc > a_limit:
                print(
                    f"Showing first {a_limit} values of {f} (total {dc} distinct values):"
                )
...
def ut_missing_key(
    ...
    print_verbose(
        1,
        a_verbose_level,
        f"unit test: checking if values for {a_column_main} in main datafrare are missing in values {a_column_lookup} of the lookup",
    )
...
def ut_check_duplicates(
    ...
    cols = ", ".join(a_columns)
    df_grouped = sql(
        f"select {cols}, count(*) as RC from {table_name} group by {cols} having RC > 1",
        a_verbose_level=a_verbose_level,
    )
    ...
    if cnt > 0:
        ...
        str_add = (
            ", showing {}:".format("all" if a_limit <= 0 else f"first {a_limit}")
            if a_show
            else ""
        )
...
def ut_check_dependent_columns(
    a_stop,
    a_df,
    a_parent_columns,
    a_children_columns,
    ...
    print_verbose(
        1,
        a_verbose_level,
        f"unit test: checking for relationship [{parent_cols}] -> [{children_cols}]",
    )

    table_name = temp_table(a_df)
    df_bad_rows = sql(
        f"select {parent_cols}, count(distinct {children_cols}) as RC from {table_name} group by {parent_cols} having RC > 1 order by RC desc",
        a_verbose_level=a_verbose_level,
    )
    ...
    if cnt > 0:
        ...
        if a_show_hist:
            pdf_hist = (
                groupby_count(
                    change(df_bad_rows, a_rename={"RC": "row_count"}), "row_count"
                )
                .toPandas()
...
def ut_null_check(a_stop, a_df, a_columns, a_verbose_level=3):
    print_verbose(
        1, a_verbose_level, f"unit test: checking for nulls for columns {a_columns}"
    )
    cnt = sql(
        "select count(*) from {0} where "
        + " or ".join(f"{c} is null" for c in a_columns),
        a_df,
        a_verbose_level=a_verbose_level,
    ).collect()[0][0]
    if cnt > 0:
...
def ut_schemas_equal(
    a_stop, a_df1: DataFrame, a_df2: DataFrame, a_check_nulls=False, a_verbose_level=3
):
...
def row_to_str(a_row: Row):
    if hasattr(a_row, "__fields__"):
        return "Row(%s)" % ", ".join(
            "%s=%r" % (k, str(v)) for k, v in zip(a_row.__fields__, tuple(a_row))
        )
    else:
...
def schema_to_str(a_schema):
    ...
    """
    return "root\n |-- " + "\n |-- ".join(
        f"{f.name}: {str(f.dataType)[:-6].lower()} (nullable = {str(f.nullable).lower()})"
        for f in a_schema.fields
    )
...
def schema_to_str_linear(a_schema):
    ...
    """
    return ", ".join(
        f"{f.name} {str(f.dataType)[:-6].lower()}" for f in a_schema.fields
    )
...
def schema_to_code(a_schema, indent=1):
    if isinstance(a_schema, StructType):
        spacer = "StructType([\n%s\n" + "    " * (indent - 1) + "])"
        return spacer % ",\n".join(
            "    " * indent + schema_to_code(field, indent + 1) for field in a_schema
        )
    elif isinstance(a_schema, StructField):
        return 'StructField("%s", %s, %s)' % (
            a_schema.name,
            schema_to_code(a_schema.dataType, indent),
            str(a_schema.nullable),
        )
    else:
        return str(a_schema) + "()"
...
def rename_columns(a_df: DataFrame, a_columns_rename_map: dict, a_verbose_level=3):
    ...
    table_name = temp_table(a_df)
    cols_str = ", ".join(
        f"{k} as {a_columns_rename_map[k]}" if k in a_columns_rename_map else f"{k}"
        for k in a_df.columns
    )
    result = _spark.sql(f"select {cols_str} from {table_name}")
    print_verbose(
        1, a_verbose_level, "Columns renamed, new schema:\n{df_schema_to_str(df)}"
    )
    return result
...
def lowercase_columns(a_df: DataFrame, a_verbose_level=3):
    ...
    return rename_columns(
        a_df, {k: k.lower() for k in a_df.columns}, a_verbose_level=a_verbose_level
    )
...
def hide_sensitive_str(d: dict):
    ...
    return (
        "{"
        + ", ".join("{}={}".format(str(k), scramble_value(k, v)) for k, v in d.items())
        + "}"
    )
...
def missing_columns(a_columns: list, a_df: DataFrame, a_lowercase=False):
    ...
    return [
        c for c in a_columns if c.lower() not in [col.lower() for col in a_df.columns]
    ]
